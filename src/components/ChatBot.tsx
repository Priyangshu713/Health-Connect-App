  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (useGemini && geminiApiKey && canUseSelectedModel) {
        const geminiSession = await createGeminiChatSession(geminiApiKey, geminiModel);
        const geminiResponse = (await geminiSession).sendMessage(input);

        const endTime = Date.now();
        const thinkingTime = endTime - startTime;

        if (isThinkingModel && (await geminiResponse).thinking) {
          const thinkingArray = (await geminiResponse).thinking
            .split(/\n(?=\d+\.|\*|\-|\(\d+\)|Step \d+:)/)
          ...
          response = {
            text: (await geminiResponse).text,
