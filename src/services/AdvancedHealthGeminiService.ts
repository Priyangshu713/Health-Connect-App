...
export const analyzeAdvancedHealthData = async (
  ...
  try {
    ...
    const  responce  = await axios.post(`${import.meta.env.VITE_BACKEND_URL_PROD}/advanced-health-analysis`,{
         age: safeData.age,
        gender: safeData.gender,
        height: safeData.height,
        weight: safeData.weight,
        bmi: safeData.bmi,
        bmiCategory: safeData.bmiCategory,
        bloodGlucose: safeData.bloodGlucose,
        modelType: modelType,
        sleepHours: safeData.sleepHours,
        sleepQuality: safeData.sleepQuality,
        exerciseHours: safeData.exerciseHours,
        ...
        lateNightSnacking: safeData.foodHabits.lateNightSnacking ? "true" : "false",
        highSugar: safeData.foodHabits.highSugar ? "true" : "false",
        fastFood: safeData.foodHabits.fastFood ? "true" : "false",
        smoking: safeData.smoking,
        alcoholConsumption: safeData.alcoholConsumption,
        medicalConditions: safeData.medicalConditions || "None reported",
        medications: safeData.medications || "None reported",
        familyHistory: safeData.familyHistory || "None reported", 
    },{
      headers: {
    ...
    if (axios.isAxiosError(error)) {
      console.error("Axios error while analyzing advanced health data:", error.message);
      if (error.response) {
