...
export const createGeminiChatSession = async (apiKey: string, modelType: GeminiModelType = "gemini-flash-lite-latest") => {
  // Validate API key
  ...
  try {
    ...
    // Define isThinkingModel based on modelType
    // const isThinkingModel = modelType.includes("thinking") || modelType === "gemini-2.5-pro-exp-03-25";
    console.log(`Creating new Gemini chat session with model: ${modelType}`);
    let chatSession = localStorage.getItem("geminiChatSession");
    if (!chatSession) {
      console.error("No chat session found in local storage. Please create a new session.");
      try {
        const controller = new AbortController();
        const newSession = await axios.post(`${import.meta.env.VITE_BACKEND_URL_PROD}/create-chat-session`,
          { modelType:modelType },
          {
            headers: {
        ...
        controller.abort();
        localStorage.setItem("geminiChatSession", newSession.data.data);
    ...
    return {
      sendMessage: async (message: string): Promise<GeminiResponse> => {
        ...
        try {
          // Send the message to your backend API and get the response
          console.log("Sending message to Gemini:", message);
          const response = await axios.post(
            `${import.meta.env.VITE_BACKEND_URL_PROD}/send-message`,
            { sessionId:chatSession, message:message},
